...
public class CliArgSpec {
	...
	public CliArgSpec(List<CliArg<?>> requiredPositionalArgs, List<CliArg<?>> optionalPositionalArgs, List<CliArg<?>> optionalArgs) {
	...
	private Map<String, CliArg<?>> getOptionalFlaggedArgs() {
	...
	private Map<String, CliArg<?>> getOptionalLabeledArgs() {
		return optionalArgs.stream()
	...
	public ParsedArgs parseArgs(String... args) throws ArgParsingFailed {
		...
		while (ix < args.length) {
			...
			ix++;
			if (arg.startsWith("--")) {
				CliArg<?> labelSpec = getOptionalLabeledArgs().get(arg);
				if (labelSpec == null) {
				...
				Map<String, CliArg<?>> optionalFlaggedArgs = getOptionalFlaggedArgs();
				CliArg<?> flagSpec = optionalFlaggedArgs.get(arg);
				if (flagSpec == null) {
